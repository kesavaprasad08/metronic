...
const UpgradePlan: FC = () => {
  ...
  const currentCurrency = useUser().user?.currency;

  useEffect(() => {
    fetch(
      `https://v6.exchangerate-api.com/v6/8f344935f20c0ffb8e0f2fff/latest/USD/`
    )
      .then((res) =>
        res.json().then((data) => {
          setExchangeRate(
            data.conversion_rates[currentCurrency ? currentCurrency : "USD"]
          );
        })
      )
  ...
  const submitHandler=():void =>{
    console.log(`selected ${selected + " pack for "+ currentState+"ly plan"}`)
  }
  return (
                          ...
                          <div className="ms-5">
                            ...
                            {!plan.custom && (
                              <>
                                ...
                                {plan.priceMonth !== undefined && plan.priceAnnual !== undefined && (
                                <span className="fs-3x fw-bolder">
                                  {currentState === "month"
                                    ? Math.floor(plan.priceMonth * exchangeRate)
                                    : Math.floor(plan.priceAnnual* exchangeRate)}
                                </span> 
                                )}
